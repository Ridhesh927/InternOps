...
async function getProofsByTask(taskId) {
  return (await pool.query('SELECT * FROM proof_submissions WHERE task_id=$1 AND deleted_at IS NULL', [taskId])).rows;
}
async function getProofsByIntern(internId) {
  return (await pool.query('SELECT * FROM proof_submissions WHERE intern_id=$1 AND deleted_at IS NULL', [internId])).rows;
}
